...
# ---------------------------------------------------------------------------
variable "name" {
  description = "Server name (becomes devops94-idp-<env>-<kind>-<name>)"
  type        = string
  validation {
...
locals {
  ...
  tags       = merge(var.tags, { resource = var.name, resource-type = "ec2-instance", environment = var.environment })
}
